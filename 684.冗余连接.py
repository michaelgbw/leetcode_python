...
class UnionFind():
    def __init__(self, edges_num):
        self.roots = [i for i in range(edges_num + 1)]
        self.rank = [0 for i in range(edges_num + 1)]
        # self.count = 0
    ...
    def union(self, p, q):
        ...
        # print('self.rank2',self.rank)
        # print('self.roots2',self.roots)
        parentP = self.find(p)
        parentQ = self.find(q)
...
ob.findRedundantConnection(edges)
